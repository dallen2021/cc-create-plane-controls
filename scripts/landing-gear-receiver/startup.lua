...
local stateFile = "landing_gear_state.txt"
...
while true do
  ...
  elseif message == "status" then
    reply(sender, "gear_" .. currentState)
  elseif message == "up" or message == "down" then
    ...
    elseif gearshift.isRunning() then
      ...
      local modifier = message == "down" and downModifier or upModifier
      ...
      if not ok then
        ...
        saveState(currentState)
        reply(sender, "gear_" .. currentState)
        drawStatus(gearshiftName, currentState, "Gear is " .. string.upper(currentState))
      end
    end
  else
